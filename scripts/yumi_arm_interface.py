...
class YumiArm(object):
    ...
    def joint_state_callback(self, data):
        ...
        self.update_ee_pose(self._joint_positions)

    """this function moves joints to a desired position"""
    def set_joint_positions(self, positions, kinematic_chain_order = True, iterate = True, timeout = 10.0):
        ...
        if iterate:
            while abs(err) > 0.07 and t < timeout:
                err = 0
                for i, p in enumerate(positions):
                    idx = i
                    if not kinematic_chain_order:
    ...
    def set_joint_position(self, idx, position, redefine_idx = True, iterate = True, timeout = 10.0):
        if redefine_idx:
        ...
        if iterate:
            t0 = rospy.Time.now().to_sec()
            t = rospy.Time.now().to_sec() - t0
            while abs(err) > 0.01 and t < timeout:
    ...
    def update_ee_pose(self, joint_pos):
        end_frame = PyKDL.Frame()
